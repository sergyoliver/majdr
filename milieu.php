...
<div class="wrapper" style=" min-height: 606px;">
    ...
    //echo $_SESSION['zone'];
    $rowdel = $bdd->prepare('select * from delegations WHERE  code_delegation =:d');
$rowdel->execute(array("d"=>$_SESSION['zone']));
$retdel = $rowdel->fetch();


$rsd = $bdd->prepare('select * from departements WHERE  delegation_code =:d');
$rsd->execute(array("d"=>$_SESSION['zone']));
$nbd = $rsd->rowCount();
?>
        <header class="head">
            <div class="main-bar row">
                <div class="col-sm-5 col-lg-6 skin_txt">
                    <h4 class="nav_top_align">
                       ...
                       DASHBORD <?php echo  $_SESSION['nomzone']?>
                    </h4>
                </div>
            ...
            <div class="card-block" style="background: transparent url(icon/main_bg.jpg) repeat 0 0;">
                ...
                <div class="row" >
                    <div class="col-lg-12 input_field_sections" >
                        ...
                        <?php
                        // nbre de departement
                        $rsp = $bdd->prepare('select * from parcelles WHERE supp =0 and delegation_code =:d   or supp is null and delegation_code =:d ');
                    ...
                    <?php
                    // nbre de departement
                    $rssup = $bdd->prepare('select sum(superficie ) as sup from parcelles WHERE supp =0 and delegation_code =:d or supp is null and delegation_code =:d');
                    $rssup->execute(array("d"=>$_SESSION['zone']));
                    $rowsup = $rssup->fetch();
                    ...
                    <div class="col-lg-12 input_field_sections">
                        ...
                        <?php
                         ...
                         // nbre de departement
                         $rsprod = $bdd->prepare('select nom from producteurs,parcelles where code_producteur=code_prod and  producteurs.supp =0  and parcelles.supp =0 and delegation_code =:d group by nom ');
                         $rsprod->execute(array("d"=>$_SESSION['zone']));
                        $rsprod->execute();
        ...
        <div class="row">
            ...
            <!--                <div style="align-content: center"><img src="icon/legende.jpg" style="text-align: center" alt=""></div>-->
                            <table  class="table table-striped  table-bordered table_res dataTable  " style="padding: 10px">
                                ...
                                <tr style=" font-size: 16px; font-weight: bold">
                                    <?php
                                        $sqlccacao = $bdd->prepare("SELECT sum(fruit_a) as a , sum(fruit_b) as b , sum(fruit_c) as c ,sum(fruit_d) as d , sum(pertes_a) as pa , sum(pertes_b) as pb  from comptage_cacaos WHERE  an_campagne = :an and delegation_code = :del and id_passage_periode = :p and supp=0  ");
                                        $sqlccacao->execute(array("an" => $row_last_passage['campagne'],"del" => $_SESSION['zone'],"p" => $row_last_passage['id']));
                    ...
                    <div class="col-lg-4 " >
                        <div class="card">
                            ...
                            <table  class="table table-striped  table-bordered table_res dataTable  " style="padding: 10px">
                                ...
                                <tr style=" font-size: 16px; font-weight: bold">
                                    <td><?php
                                        $sqlcp = $bdd->prepare("SELECT sum(grappe) as gp , sum(fruit) as f  from comptage_cafes WHERE  an_campagne = :an and delegation_code = :del and id_passage_periode = :p  ");
                                        $sqlcp->execute(array("an" => $row_last_passagec['campagne'],"del" => $_SESSION['zone'],"p" => $row_last_passagec['id']));
                                        $rowcp = $sqlcp->fetch();
            ...
            <div class="col-lg-8 m-t-35">
                <table class="table table-striped  table-bordered table_res dataTable  " style="padding: 10px">
                    ...
                    <?php
                    $idel=1;
                    $sqlccacao_dr = $bdd->prepare("SELECT count(id) as a, delegation_code  from comptage_cacaos WHERE  an_campagne = :an  and id_passage_periode = :p and delegation_code = :dl and supp=0  group by delegation_code ");
                    $sqlccacao_dr->execute(array("an" => $row_last_passage['campagne'],"p" => $row_last_passage['id'],"dl" =>$_SESSION['zone']));
