...
?>
    ...
    $i=1;
    if ($vil=='vide'){
        $rsdel = $bdd->prepare('select parcelle_code from comptage_cafes where  an_campagne = :c  and departement_code = :dp  and id_passage_periode = :p  GROUP BY parcelle_code ');
        $rsdel->execute(array('c'=>$camp,'dp'=>$dep,'p'=>$pas));
    }else{
        $rsdel = $bdd->prepare('select parcelle_code from comptage_cafes where  an_campagne = :c  and departement_code = :dp and village_code = :v  and id_passage_periode = :p  GROUP BY parcelle_code ');
