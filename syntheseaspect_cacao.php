...
if (isset($_POST['camp'])  ) {
   $periode= $_POST['p'];
    ...
    function liste_passage($camp,$periode)
    {
        include '../connexion/connectpg.php';
        // include '../connexion/function.php';
        $rsan = $bdd->prepare("SELECT comptage_cacaos.id_passage_periode FROM comptage_cacaos,passage_periodes WHERE passage_periodes.id=comptage_cacaos.id_passage_periode and passage_periodes.type_periode= :tp AND  an_campagne =:an and comptage_cacaos.supp=0  GROUP BY comptage_cacaos.id_passage_periode,type_periode ");
...
//echo "select CAST(sum(fe) as float)/count(idpassage)  as x, CAST(sum(flo) as float)/count(idpassage)  as y, CAST(sum(Noue) as float)/count(idpassage)  as r FROM $nomtab where comptage_cacaos.code_dep = '$codedp' and comptage_cacaos.supp=0 and comptage_cacaos.idpassage=$idp and comptage_cacaos.an_campagne ='$an'";
       // echo "select CAST(sum(fe) as float)/count(idpassage)  as x, CAST(sum(flo) as float)/count(idpassage)  as y, CAST(sum(Noue) as float)/count(idpassage)  as r,avg(CAST(sum(fe) as float)/count(idpassage)) as moyx FROM $nomtab where comptage_cacaos.code_dep = '$codedp' and comptage_cacaos.supp=0 and comptage_cacaos.idpassage=$idp and comptage_cacaos.an_campagne ='$an'";
        $rsc = $bdd->prepare("select sum(fe)/count(id_passage_periode)  as x, sum(flo)/count(id_passage_periode)  as y, sum(Noue)/count(id_passage_periode)  as r
FROM $nomtab where comptage_cacaos.departement_code = '$codedp' and comptage_cacaos.supp=0 and comptage_cacaos.id_passage_periode=$idp and comptage_cacaos.an_campagne ='$an'");
        $rsc->execute();
...
function somme_cpte_del2($nomtab, $codedp, $idp, $an)
    ...
    $new_an_1= $and."-".$anf;
        //$codeapp = 'K' . $rest;
     //  echo  "select sum(fe) /count(id_passage_periode)  as x, sum(flo)/count(id_passage_periode)  as y, sum(Noue)/count(id_passage_periode)  as r FROM $nomtab where comptage_cacaos.departement_code = '$codedp' and comptage_cacaos.supp=0 and comptage_cacaos.id_passage_periode=$idp and comptage_cacaos.an_campagne ='$new_an_1'";

       $rsc = $bdd->prepare("select sum(fe) /count(id_passage_periode)  as x, sum(flo)/count(id_passage_periode)  as y, sum(Noue)/count(id_passage_periode)  as r FROM $nomtab where comptage_cacaos.departement_code = '$codedp' and comptage_cacaos.supp=0 and comptage_cacaos.id_passage_periode=$idp and comptage_cacaos.an_campagne ='$new_an_1'");
        $rsc->execute();
        $rowc = $rsc->fetchAll(PDO::FETCH_ASSOC);
        return $rowc;
    }
    ...
    $tabliste_1 = liste_passage($new_an_1,$periode);
           ...
           // $totx=0;
            ...
            for ($zt = 1; $zt <= $nbre_decomptage; $zt++) {
                ...
                if ($zt == 1) {
                    ...
                    $list_1 = somme_cpte_del2('comptage_cacaos', $codedep, $tabliste_1[0]['id_passage_periode'], $_POST['camp']);

                }
             if ($zt == 2) {
                    $list2 = somme_cpte_del('comptage_cacaos', $codedep, $tabliste[1]['id_passage_periode'], $_POST['camp']);
                    $list_2 = somme_cpte_del2('comptage_cacaos', $codedep, $tabliste_1[1]['id_passage_periode'], $_POST['camp']);
                }
                if ($zt == 3) {
                    $list3 = somme_cpte_del('comptage_cacaos', $codedep, $tabliste[2]['id_passage_periode'], $_POST['camp']);
                    $list_3 = somme_cpte_del2('comptage_cacaos', $codedep, $tabliste_1[2]['id_passage_periode'], $_POST['camp']);

                }
                if ($zt == 4) {
                    $list4 = somme_cpte_del('comptage_cacaos', $codedep, $tabliste[3]['id_passage_periode'], $_POST['camp']);
                    $list_4 = somme_cpte_del2('comptage_cacaos', $codedep, $tabliste_1[3]['id_passage_periode'], $_POST['camp']);

                }
                if ($zt == 5) {
                    $list5 = somme_cpte_del('comptage_cacaos', $codedep, $tabliste[4]['id_passage_periode'], $_POST['camp']);
                    $list_5 = somme_cpte_del2('comptage_cacaos', $codedep, $tabliste_1[4]['id_passage_periode'], $_POST['camp']);

                }
                if ($zt == 6) {
                    $list6 = somme_cpte_del('comptage_cacaos', $codedep, $tabliste[5]['id_passage_periode'], $_POST['camp']);
                    $list_6 = somme_cpte_del2('comptage_cacaos', $codedep, $tabliste_1[5]['id_passage_periode'], $_POST['camp']);
