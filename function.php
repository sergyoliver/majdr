...
function generer_codepar($type,$dep){
    ...
    $rowdp = $rsdp->fetch();
    $codedel = $rowdp['codedep'];
    if ($type='CAFE'){
        $init = 'C';
    }else{
        $init = 'K';
    }
    $codedelr = $init.$rowdp['codedep'];
//echo "select * from tb_parcelle where type_plantation= '$type' and code_parcelle LIKE '$codedelr%'";

    $rs1234 = $bdd->prepare("select * from parcelles where type_plantation= :type and code_parcelle LIKE '$codedelr%'");
...
function separer_montant($str){
    $mnt =  number_format($str,0,","," ");
    return $mnt;
}
?>