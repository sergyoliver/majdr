...
if (isset($_POST['camp'])  ) {
   $periode= $_POST['p'];
    ...
    function liste_passage($camp,$periode)
    {
        include '../connexion/connectpg.php';
        // include '../connexion/function.php';
        $rsan = $bdd->prepare("SELECT id_passage_periode FROM comptage_cacaos,passage_periodes WHERE passage_periodes.id=comptage_cacaos.id_passage_periode and passage_periodes.type_periode= :tp AND  an_campagne =:an  and delegation_code =:d  and comptage_cacaos.supp=0 GROUP BY id_passage_periode,type_periode ");
        $rsan->execute(array("an" => $camp,"tp" => $periode,"d" => $_SESSION['zone']));
       // $rowan = $rsan->rowCount();
   ...
   // echo count($tabliste);
    ...
    <tbody>
    <?php
    $sqldel = $bdd->prepare("SELECT delegations.code_delegation,delegations.designation FROM delegations,comptage_cacaos WHERE comptage_cacaos.delegation_code=code_delegation and delegation_code =:d and comptage_cacaos.supp=0 GROUP BY delegations.code_delegation,delegations.designation ORDER BY delegations.designation ASC  ");
    $sqldel->execute(array("d" => $_SESSION['zone']));
